...
//         alert(`All fields are required for Parent ${i+1}`);
//         return;
//       }
//         // return `All fields are required for Parent ${i+1}`;
//       if (!PHONE_RX.test(p.phone)){
//         alert(`Invalid phone for Parent ${i+1}`);
//         return;
//       }
//         // return `Invalid phone for Parent ${i+1}`;
//       if (!EMAIL_RX.test(p.email)) {
//         alert(`Invalid e-mail for Parent ${i+1}`);
//         return;
//       }
//       //  return `Invalid e-mail for Parent ${i+1}`;
//       if (!BLOOD_RX.test(p.bloodGroup)){
//         alert(`Invalid blood group for Parent ${i+1}`);
//         return;
//       }
//         // return `Invalid blood group for Parent ${i+1}`;
//       if (new Date(p.birthDate) > new Date()){
//         alert(`Parent ${i+1} birth date cannot be in the future`);
//         return;
//       }
//         // return `Parent ${i+1} birth date cannot be in the future`;
//       if (yearsBetween(p.birthDate,TODAY_ISO) < 18){
//         alert( `Parent ${i+1} must be ≥ 18 years old`);
//         return;
//       }
...
export default function FamilyForm() {
  ...
  const handleSubmit = e =>{
    ...
    const msg = validate();
    if (msg) {
      setError(msg);                // show banner
      alert(msg);                   // optional native pop‑up
